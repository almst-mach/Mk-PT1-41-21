import decimal  # имеем дело с деньгами
...
def get_debt(inp_data):  # передаем список - где под индексом 0 - общая сумма, 1 - количество персон, остальные - это суммы персон
    ...
    sum_each = (decimal.Decimal(str(inp_data[0])) / decimal.Decimal(str(inp_data[1])))  # сумму чека делим на количество персон
    person_amount = {f'Person №{i - 1}': decimal.Decimal(str(inp_data[i])) for i in range(2, len(inp_data))}  # создаем словарь с каждой персоной и её суммой
    ...
    for k, v in person_amount.items():  # алгоритм по наполнению словарей creditors и debtors
        if v > sum_each:  # сумма больше средней - ему должны
            creditors[k] = decimal.Decimal(str(v)) - decimal.Decimal(str(sum_each))  # сумма которую кредитору должны
        elif v < sum_each:  # сумма меьшн средней - он должен
            debtors[k] = decimal.Decimal(str(sum_each)) - decimal.Decimal(str(v))  # сумма которую должник должен
        ...
        while v != 0:  # пока не обнулим долг кредитору итерируемся по должникам ниже
            for k1, v1 in debtors.items():
                ...
                if v >= v1:  # если данному кредитору должны больше чем должен данный должник
                    v = decimal.Decimal(str(v)) - decimal.Decimal(str(v1))  # отнимаем долг
                    ...
                    v1 = decimal.Decimal(str(v1)) - decimal.Decimal(str(v))  # отнимаем сумму кредитора
