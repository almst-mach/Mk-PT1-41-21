...
minutes_dict_case_three = {'45':'пятнадцати','46':'четырнадцати','47':'тринадцати','48':'двенадцати','49':'одинадцати','50':'десяти','51':'девяти','52':'девяти','53':'семи','54':'шести','55':'пяти','56':'четырех','57':'трех','58':'двух','59':'одной',}
        #Сделал три словаря с минутами под каждый диапозон времени и один часовой
        #В часовом словаре ключ делал с разницей в единицу потому что в задании мы ссылаемся на следующий час
if 0 <= int(minutes) < 30:
    if int(minutes) == 2 or int(minutes) == 3 or int(minutes) == 4 or int(minutes) == 22 or int(minutes) == 23 or int(minutes) == 24:
        print(f"{get_value(minutes_dict_case_one, minutes)} минуты {get_value(hours_dict, hours)}") #Хотел сделать через or и остаток от 
    elif int(minutes) == 1 or int(minutes) == 21:                                                   #деления,но 12,13,14 будут портить вывод
        print(f"{get_value(minutes_dict_case_one, minutes)} минута {get_value(hours_dict, hours)}")
    else:  
        print(f"{get_value(minutes_dict_case_one, minutes)} минут {get_value(hours_dict, hours)}")
elif int(minutes) == 30:
    print(f"половина {get_value(hours_dict, hours)}")
    ...
    if int(minutes) == 42 or int(minutes) == 43 or int(minutes) == 44 or int(minutes) == 32 or int(minutes) == 33 or int(minutes) == 34:
        print(f"{get_value(minutes_dict_case_two, minutes)} минуты {get_value(hours_dict, hours)}")
    elif int(minutes)%10 == 1:
        print(f"{get_value(minutes_dict_case_two, minutes)} минута {get_value(hours_dict, hours)}")
    else:
        print(f"{get_value(minutes_dict_case_two, minutes)} минут {get_value(hours_dict, hours)}")
else:
    if int(minutes) == 1:
        print(f"Без {get_value(minutes_dict_case_three, minutes)} минуты {get_value(hours_dict, hours)}")
    else:
        print(f"Без {get_value(minutes_dict_case_three, minutes)} минут {get_value(hours_dict, hours)}")
